...
class Dataset_From_Numpy(Dataset):
    ...
    def __read_data__(self):
        print("从预处理好的Numpy文件中加载数据...")
        # 从文件中加载 X 和 y
        self.data_x = np.load(os.path.join(self.root_path, 'final_X.npy'))
        self.data_y = np.load(os.path.join(self.root_path, 'final_y.npy'))
        ...
        # TimeMixerPP需要时间戳特征(x_mark), 即使我们不用，也需要提供一个占位符
        # 创建一个形状匹配的零张量作为x_mark和y_mark
        self.data_stamp = np.zeros((self.data_x.shape[0], self.data_x.shape[1], 4))
    ...
    def __getitem__(self, index):
        ...
        # y_mark 的长度需要匹配 seq_y 的长度
        seq_y_mark = np.zeros((self.pred_len, 4))

        return torch.FloatTensor(seq_x), torch.FloatTensor(seq_y), torch.FloatTensor(seq_x_mark), torch.FloatTensor(seq_y_mark)
